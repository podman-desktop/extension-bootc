...
// Validation to check that the image path ends with .raw or .qcow2
$effect(() => {
...
// Validation for the virtual machine name, each time it changes or existingVMNames is updated, make sure it does not match any existing VM names
$effect(() => {
  if (virtualMachineName || existingVMNames) {
    const isNameTaken = existingVMNames.includes(virtualMachineName);
    if (isNameTaken) {
      errorFormValidation =
...
async function listNamesOfVms(): Promise<string[]> {
  try {
    const vms = await bootcClient.macadamListVMs();
...
async function createVM(): Promise<void> {
  ...
  createErrorMessage = '';
  try {
    await bootcClient.macadamCreateVM({
      imagePath,
        ...
        <div class={createInProgress ? 'opacity-40 pointer-events-none' : ''}>
          <div class="pb-4">
            <p class="text-sm text-[var(--pd-content-text)]">
              ...
              the VM using the below credentials.
            </p>
          </div>
          <div class="pb-4">
