...
export class MacadamVirtualMachine {
  ...
  async init(): Promise<void> {
    try {
      ...
      console.error('Failed to initialize Macadam:', err);
      throw new Error(`Macadam init failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
